...
class ClientDetailsPage extends StatefulWidget {
  ...
  const ClientDetailsPage({Key? key, required this.clientData}) : super(key: key);
...
class _ClientDetailsPageState extends State<ClientDetailsPage> {
  ...
  void _initializeControllers() {
    ...
    _phoneController = TextEditingController(text: widget.clientData['phone']?.toString());
  ...
  Future<void> _saveChanges() async {
    if (!OfflineActionHelper.canPerformAction(context, actionName: 'update client')) {
      return;
    }

    if (_nameController.text.trim().isEmpty) {
      AppToast.showError(context: context, title: 'Error', message: 'Name cannot be empty');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          widget.clientData['phone'] = int.tryParse(_phoneController.text.trim());
          widget.clientData['notes'] = _notesController.text.trim();
        });
        AppToast.showSuccess(context: context, title: 'Success', message: 'Client updated successfully');
      }
    } catch (e) {
      if (mounted) {
        setState(() => _isLoading = false);
        AppToast.showError(context: context, title: 'Error', message: 'Failed to update client');
      }
    }
  }

  Future<void> _deleteClient() async {
    if (!OfflineActionHelper.canPerformAction(context, actionName: 'delete client')) {
      return;
    }
    ...
    try {
      await _supabase.from('clients').delete().eq('id', widget.clientData['id']);
      if (mounted) {
        AppToast.showSuccess(context: context, title: 'Success', message: 'Client deleted successfully');
        Navigator.pop(context, true); // Return true to indicate deletion
      }
    } catch (e) {
      if (mounted) {
        setState(() => _isLoading = false);
        AppToast.showError(context: context, title: 'Error', message: 'Failed to delete client');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _isEditing ? 'Edit Client' : 'Client Profile',
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const Text('Offline Mode: Editing is disabled', style: TextStyle(color: Color(0xFF92400E))),
                  ],
                ),
              ),
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(
                    fontSize: 15,
                    color: controller.text.isEmpty ? Colors.grey[400] : const Color(0xFF1F2937),
                    height: 1.4,
                  ),
                ),
              ],
